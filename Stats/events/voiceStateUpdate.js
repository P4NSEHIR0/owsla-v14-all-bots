...
client.on("ready", async () => {
  let channels = client.guilds.cache.get(client.ayarlar.sunucuId).channels.cache.filter(channel => channel.type == "GUILD_VOICE" && channel.members.size > 0);
...
client.on("ready", async () => {
  ...
  setInterval(() => {
    sunucu.members.cache.filter(x => !x.user.bot && x.me.voice.channel && x.me.voice.channel.id != data.SLEEP && data.PUBCategory.includes(x.me.voice.channel.parentID) && x.roles.cache.get(rol)).map(user => {
      ...
      if (Date.now() - data.time >= 1000 * 60 * 5 && data.deaf == true) {
        ...
        user.me.voice.setChannel(data.SLEEP)
      }
    })
  }, 10000)
});
...
client.on("voiceChannelDeaf", (member, deafType) => {
  ...
  client.channelTime.set(member.id, {
    channel: member.me.voice.channel.id,
    time: Date.now(),
    deaf: true
  })
});
...
client.on("voiceChannelJoin", async (member, channel) => {
  ...
  if (!client.channelTime.has(member.id)) {
    client.channelTime.set(member.id, {
      channel: channel.id,
      time: Date.now(),
      deaf: member.me.voice.deaf == true ? true : false
    })
  }
  ...
  if (!vmuted && user) {
    ...
    member.me.voice.setMute(false).catch(() => {});
  } else if (vmuted && !user) {
    member.roles.add(rol).catch(() => {});
    member.me.voice.setMute(true).catch(() => {});
  }
    ...
    Stat.findOne({
      ...
      if (!res) {
        ...
        */
        ...
        let streamer = client.channels.cache.filter(x => x.parentID === client.ayarlar.STREAMER_KATEGORI && x.type == "GUILD_VOICE").map(x => x.id);
        let pub = client.channels.cache.filter(x => x.parentID === client.ayarlar.PUBLIC_KATEGORI && x.type == "GUILD_VOICE" && x.id !== client.ayarlar.SLEEP_ROOM).map(x => x.id);
        let kayit = client.channels.cache.filter(x => x.parentID === client.ayarlar.REGISTER_KATEGORI && x.type == "GUILD_VOICE").map(x => x.id);
        ...
        let siradakilevel = res.me.voiceLevel * 643;
        if (siradakilevel <= res.me.voiceXP) {
          res.me.voiceLevel++
        }
        res.save();
      };
