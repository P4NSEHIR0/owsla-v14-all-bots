...
**${message.guild.name}** adlı sunucunun \`TOP - 10\` **Toplam** istatistik verileri listelenmektedir!`)
                  .addFields([
                      { name: `**En Fazla Coin Kasanlar**`, value: `Toplam yapılan coin miktarı: \`${cmiktar.toFixed(0)} coin\`\n${coinSira}` },
                      { name: `**Toplam Ses Sıralaması**`, value: `Toplam ses aktifliği: \`${moment.duration(sesMiktar).format("H [saat,] m [dk.]")}\`\n${toplamSesSiralama ? `${toplamSesSiralama}${benimSesSira}` : "Datacenter'da kaydedilen bir veri görüntülenemedi!"}` },
                      { name: `**Toplam Ses Kanal Sıralaması**`, value: `Toplam ses kanal aktifliği: \`${moment.duration(sesMiktar).format("H [saat,] m [dk.]")}\`\n${sirali.join("\n")}` },
                      { name: `**Toplam Mesaj Sıralaması**`, value: `Toplam gönderilen mesaj sayısı: \`${CHATMiktar} mesaj\`\n${toplamCHATSiralama ? `${toplamCHATSiralama}${benimMesajSira}` : "Datacenter'da kaydedilen bir veri görüntülenemedi!"}` },
                      { name: `**Toplam Mesaj Kanal Sıralaması**`, value: `Toplam gönderilen kanal mesaj sayısı: \`${CHATMiktar} mesaj\`\n${sirali2.join("\n")}` }
                  ]);
