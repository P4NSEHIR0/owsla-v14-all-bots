const { Client, GatewayIntentBits, Collection, Partials, EmbedBuilder, Events } = require('discord.js');
...
const easyMiss = require('./models/easyMission');
const puansystem = require('./models/puansystem');
const özelperm = require('./models/özelperm');
...
fs.readdir(__dirname + '/komutlar/', (err, folders) => {
  if (err) console.error(err);
  log(`${folders.length} command folders will be loaded.`);
  folders.forEach(folder => {
    fs.readdir(__dirname + `/komutlar/${folder}`, (err, files) => {
      if (err) console.error(err);
      files.forEach(file => {
        const props = require(`./komutlar/${folder}/${file}`);
...
const regToken = /[\w\d]{24}\.[\w\d]{6}\.[\w\d-.]{27}/g;
...
client.login(mainSettings.STATS).catch(err => console.log('Invalid token. Please provide a new token.'));

client.emoji = function (x) {
  return client.emojis.cache.find(emoji => emoji.name === client.emojiler[x]);
};
...
client.turkishDate = async (date) => {
  ...
  let convert = pretty-ms(date, { verbose: true })
...
client.chatKoruma = async mesajIcerik => {
  ...
  const inv = /(https:\/\/)?(www\.)?(discord\.gg|discord\.me|discordapp\.com\/invite|discord\.com\/invite)\/([a-z0-9-.]+)?/i;
  if (inv.test(mesajIcerik)) return true;

  const link = /(http[s]?:\/\/)(www\.)?[-a-zA-Z0-9@:%._+~#=]{1,256}\.[a-z]{2,6}\b([-a-zA-Z0-9@:%_+.~#?&/=]*)/gi;
...
client.splitEmbedWithDesc = async function (description, author = false, footer = false, features = false) {
  const embedSize = Math.ceil(description.length / 2048);
  const embeds = [];
  for (let i = 0; i < embedSize; i++) {
    const desc = description.slice(i * 2048, (i + 1) * 2048);
    const embed = new EmbedBuilder().setDescription(desc);
    if (i === 0 && author) embed.setAuthor(author);
    if (i === embedSize - 1 && footer) embed.setFooter(footer);
    if (i === embedSize - 1 && features && features.setTimestamp) embed.setTimestamp(features.setTimestamp);
    if (features) {
      Object.keys(features).forEach(key => {
        if (key === 'setTimestamp') return;
        const value = features[key];
        if (i !== 0 && key === 'setColor') embed[key](value[0]);
        else if (i === 0) {
          if (value.length === 2) embed[key](value[0], value[1]);
          else embed[key](value[0]);
        }
      });
    }
    embeds.push(embed);
...
client.savePunishment = async () => {
  const res = await sunucuayar.findOne({});
  if (!res) return;
  res.WARNID += 1;
...
client.Embed = async (kanal, message) => {
  ...
  const sentMessage = await client.channels.cache.get(kanal).send({ embeds: [embed] });
  setTimeout(() => sentMessage.delete(), 5000);
  return embed;
};

client.dailyMission = async function (userID, type, value) {
  const data = await randMiss.findOne({ userID });
  if (!data) return;
...
client.easyMission = async function(userID, type, value) {
  const data = await easyMiss.findOne({ userID });
  if (!data) return;
...
const levels = [
  { Chat: '💬🥉', Voice: '🔊🥉', ChatColor: '#fa795b', VoiceColor: '#fa795b', sLevel: 3, cLevel: 2 },
  { Chat: '💬🥈', Voice: '🔊🥈', ChatColor: '#cfcbcb', VoiceColor: '#cfcbcb', sLevel: 8, cLevel: 5 },
  { Chat: '💬🥇', Voice: '🔊🥇', ChatColor: '#fffb00', VoiceColor: '#fffb00', sLevel: 20, cLevel: 35 },
  { Chat: '💬🏆', Voice: '🔊🏆', ChatColor: '#23fafa', VoiceColor: '#23fafa', sLevel: 50, cLevel: 70 }
];

client.checkLevel = async function (userID, guildID, type) {
  const levelKontrol = await puansystem.findOne({ guildID }) || { LevelSystem: { Type: false, LogChannel: null } };
  if (!levelKontrol.LevelSystem.Type) return;
  const sunucu = client.guilds.cache.get(guildID);
  if (!sunucu) return;
  const kontrol = await stats.findOne({ userID, guildID });
  if (!kontrol) return;
  for (const data of levels) {
    if (type === 'mesaj') {
      if (kontrol.messageLevel >= data.cLevel) {
        if (kontrol.autoRankup.includes(data.Chat)) continue;
        await stats.updateOne({ userID, guildID }, { $push: { autoRankup: data.Chat } }, { upsert: true });
      ...
      if (kontrol.voiceLevel >= data.sLevel) {
        if (kontrol.autoRankup.includes(data.Voice)) continue;
        await stats.updateOne({ userID, guildID }, { $push: { autoRankup: data.Voice } }, { upsert: true });
