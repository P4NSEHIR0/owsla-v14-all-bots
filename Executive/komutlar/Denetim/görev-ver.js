...
`)
                .addFields({ name: `Görev Yapanlar`, value: `${görevYapanlar ? görevYapanlar : "Yapılan görev bulunmamakta"}` })
                .addFields({ name: `Devam Eden Görevler`, value: `${devamEdenGörev}` })
            ]
            });
        }
        if (["sil"].includes(sec)) {
            ...
            if (["hepsi", "tümü"].includes(sec2)) {
                data.filter(data => data.Check >= data.Mission.Amount || Date.now() >= data.Time).forEach(data => {
                    easyMiss.deleteOne({
                        userID: data.userID
                    }).exec();
                });
    ...
    if (!sec) {
        let data = await easyMiss.find({});
        let authorGörev = await easyMiss.findOne({
...
function progressBar(value, maxValue, size) {
    ...
    const progressText = `${client.emojis.cache.find(x => x.name == "stfu_ortabar")}`.repeat(progress);
    const emptyProgressText = `${client.emojis.cache.find(x => x.name == "stfu_griortabar")}`.repeat(emptyProgress);
    const bar = `${value ? client.emojis.cache.find(x => x.name == "stfu_solbar") : client.emojis.cache.find(x => x.name == "stfu_baslangicbar")}` + progressText + emptyProgressText + `${emptyProgress == 0 ? `${client.emojis.cache.find(x => x.name === "stfu_bitisbar")}` : `${client.emojis.cache.find(x => x.name === "stfu_gribitisbar")}`}`;
    return bar;
}
