...
module.exports.run = async (client, message, args, durum, kanal) => {
    ...
    let members = client.guilds.cache.get(client.ayarlar.sunucuId).roles.cache.get(enAltYetkiliRol).members.map(member => member);
    ...
    let VoiceChannel = client.guilds.cache.get(client.ayarlar.sunucuId).channels.cache.filter(chan => chan.type === ChannelType.GuildVoice && kategoriler.includes(chan.parentId) && !yasaklıkanal.includes(chan.id)).map(channel => channel.id);
    let MessageChannel = client.guilds.cache.get(client.ayarlar.sunucuId).channels.cache.filter(chan => chan.type === ChannelType.GuildText && messageKategori.includes(chan.id)).map(channel => channel.id);
