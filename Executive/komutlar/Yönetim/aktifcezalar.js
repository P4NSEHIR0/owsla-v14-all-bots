...
Toplamda **${karantina}** kişi hesabını **7 Gün**'den önce açılmış.
        ...
        if (sec == "mute") {
            ...
            message.channel.send({
                content: `${muted.length > 0 ? muted.map(x => `<@${x.userID}> - Bitiş Süresi ${moment(x.endDate).locale("tr").format("LLL")}`).join("\n") : "Aktif Mute Cezası Görüntülenemiyor"}`,
                split: true
            });
        }
        if (sec == "vmute") {
        ...
        if (sec == "jail") {
            ...
            message.channel.send({
                content: `${jailed.length > 0 ? jailed.filter(x => x.endDate != null).map(x => `<@${x.userID}> - Bitiş Süresi ${x.endDate == null ? "KALICI" : moment(x.endDate).locale("tr").format("LLL")}`).join("\n") : "Aktif Mute Cezası Görüntülenemiyor"}`,
                split: true
            });
        }
        if (sec == "kaldır") {
            ...
            muted.forEach(async (data, index) => {
                ...
                if (!member) {
                    await muteInterval.deleteOne({ userID: data.userID }).exec();
                    return message.channel.send(`(\`${data.userID}\`) adlı üyenin süresi dolduğu için verileri sıfırlandı`);
                }
                setTimeout(async () => {
                    await muteInterval.deleteOne({ userID: data.userID }).exec();
            ...
            vmuted.forEach(async (data, index) => {
                let member = message.guild.members.cache.get(data.userID);
                if (!member) {
                    await vmuteInterval.deleteOne({ userID: data.userID }).exec();
                    return message.channel.send(`(\`${data.userID}\`) adlı üyenin süresi dolduğu için verileri sıfırlandı`);
                }
                setTimeout(async () => {
                    await vmuteInterval.deleteOne({ userID: data.userID }).exec();
            ...
            jailed.forEach(async (data, index) => {
                let member = message.guild.members.cache.get(data.userID);
                if (!member) {
                    await jailInterval.deleteOne({ userID: data.userID }).exec();
                    return message.channel.send(`(\`${data.userID}\`) adlı üyenin süresi dolduğu için verileri sıfırlandı`);
                }
                setTimeout(async () => {
                    await jailInterval.deleteOne({ userID: data.userID }).exec();
                    message.channel.send(`<@${data.userID}> (\`${data.userID}\`) adlı üyenin süresi dolduğu için verileri sıfırlandı`);
                    await member.roles.set(member.roles.cache.get(sdata.BOOST) ? sdata.UNREGISTER.concat(sdata.BOOST) : sdata.UNREGISTER);
                }, index * 1000);
            });
        }
    }
