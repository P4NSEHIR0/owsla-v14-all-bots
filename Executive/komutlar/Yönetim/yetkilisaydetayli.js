...
module.exports.run = async (client, message, args, durum, kanal) => {
    ...
    if (message.member.permissions.has(PermissionsBitField.Flags.Administrator) && durum) {
        ...
        message.channel.send({
            content: `Toplam: ${alt_yönetim_ses + orta_yönetim_ses + üst_yönetim_ses} kişi | Seste Olmayan: ${alt_yönetim_ses_olmayan.length + orta_yönetim_ses_olmayan.length + üst_yönetim_ses_olmayan.length} kişi`,
            code: "md"
        }).then(x => {
            message.channel.send({
                content: `Alt yönetim: (${alt_yönetim_ses}) kişi | Alt yönetimde toplamda (${alt_yönetim_ses_olmayan.length}) kişi seste değil.`,
                code: "md",
                split: true
            }).then(x => {
                message.channel.send({
                    content: `${alt_yönetim_ses_olmayan.length > 0 ? alt_yönetim_ses_olmayan : "Tüm aktif yetkililer ses kanallarında görünüyor."}`,
                    code: "md",
                    split: true
                }).then(x => {
                    message.channel.send({
                        content: `Orta yönetim: (${orta_yönetim_ses}) kişi | Orta yönetimde toplamda (${orta_yönetim_ses_olmayan.length}) kişi seste değil.`,
                        code: "md",
                        split: true
                    }).then(x => {
                        message.channel.send({
                            content: `${orta_yönetim_ses_olmayan.length > 0 ? orta_yönetim_ses_olmayan : "Tüm aktif yetkililer ses kanallarında görünüyor."}`,
                            code: "md",
                            split: true
                        }).then(x => {
                            message.channel.send({
                                content: `Üst yönetim: (${üst_yönetim_ses}) kişi | Üst yönetimde toplamda (${üst_yönetim_ses_olmayan.length}) kişi seste değil.`,
                                code: "md",
                                split: true
                            }).then(x => {
                                message.channel.send({
                                    content: `${üst_yönetim_ses_olmayan.length > 0 ? üst_yönetim_ses_olmayan : "Tüm aktif yetkililer ses kanallarında görünüyor."}`,
                                    code: "md",
                                    split: true
                                });
                            });
