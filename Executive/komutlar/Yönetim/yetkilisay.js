...
module.exports.run = async (client, message, args, durum, kanal) => {
    ...
    if (message.member.permissions.has(PermissionsBitField.Flags.Administrator) && message.member.roles.cache.some(rol => data[0].UstYetkiliRol.some(rol2 => rol.id === rol2)) || durum) {
        ...
        if (!sec) {
            ...
            let Kisuke = message.guild.members.cache.filter(member => {
                return member.roles.cache.has(enAltYetkiliRol) && !member.voice.channel && member.presence?.status !== "offline" && !member.user.bot && !client.ayarlar.sahip.includes(member.user.id);
            }).size;
            ...
            message.channel.send({ content: table.create(tablo), split: true, code: "md" });
            message.channel.send({ content: Kisukea, split: { char: "," }, code: "md" });
        }
    } else {
        return;
    }
