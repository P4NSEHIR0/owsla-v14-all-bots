...
`)
    ...
    if (sec === "hepsi" || sec === "tümkanallar") {
      let Kisukea = message.guild.members.cache
        .filter((member) => {
          return (
...
**Kazanan Kullanıcı:** <@${random}> (\`${random}\`)`)
    ...
    if (sec === "bulundugum" || sec === "bulunduğum") {
      ...
      let Kisukea = message.member.voice.channel.members
        .filter((member) => {
          return (
            member.voice.channel &&
            member.presence?.status !== "offline" &&
...
**Kazanan Kullanıcı:** <@${random}> (\`${random}\`)`)
    ...
    if (!args[0].endsWith("d") && !args[0].endsWith("h") && !args[0].endsWith("m"))
      return message.channel.send(`Zaman için doğru biçimlendirmeyi kullanmadınız! (\`m/h/d\`)`);
    if (isNaN(args[0][0])) return message.channel.send(`Girmiş olduğun değer bir rakam olmalı!`);
    let channel = message.mentions.channels.first();
    if (!channel) return message.channel.send(`Lütfen bir kanal belirtiniz!`);
    ...
    let Embed = new Discord.EmbedBuilder()
      ...
      .setDescription(`
Çekilişe katılmak için 🎉 tepkisine tıklayınız
\`\`\`
      ...
      if (saniye === 0 || saniye < 0) {
        setTimeout(() => {
          if (m.reactions.cache.get("🎉").count <= 2) {
            ...
            return message.channel.send(`Çekilişin sonuçlanabilmesi için yeterli sayıda katılımcı olmadı`);
          }

          m.reactions.cache
            .get("🎉")
            .users.cache.filter((u) => !u.bot && message.author.id !== u.id)
            .map((x) => {
