...
module.exports.run = async (client, message, args, durum, kanal) => {
    ...
    if (sunucuData.GKV.includes(message.author.id) || message.author.id == message.guild.ownerId || client.ayarlar.sahip.includes(message.author.id)) {
        let sec = args[0];
        ...
        if (["oluştur"].includes(sec)) {
            ...
            message.channel.send({
                embeds: [new EmbedBuilder()
                    ...
                    .setTimestamp()
                    .setDescription(`\`${komutAd}\` adlı komut başarılı bir şekilde oluşturuldu.
        ...
        if (["güncelle"].includes(sec)) {
            ...
            if (["verilcekler"].includes(args[2])) {
                ...
                if (ozelKomutDATA.verilcekRol.includes(role.id)) {
                    ...
                    await ozelKomut.updateOne({ guildID: message.guild.id, komutAd: args[1] }, { $pull: { verilcekRol: role.id } }).exec();
                } else {
                    message.reply("başarılı bir şekilde etiketlediğiniz rolü ekledim.");
                    await ozelKomut.updateOne({ guildID: message.guild.id, komutAd: args[1] }, { $push: { verilcekRol: role.id } }).exec();
                }
            }
            if (["roller"].includes(args[2])) {
                ...
                if (ozelKomutDATA.roller.includes(role.id)) {
                    message.reply("başarılı bir şekilde etiketlediğiniz rolü kaldırdım.");
                    await ozelKomut.updateOne({ guildID: message.guild.id, komutAd: args[1] }, { $pull: { roller: role.id } }).exec();
                } else {
                    message.reply("başarılı bir şekilde etiketlediğiniz rolü ekledim.");
                    await ozelKomut.updateOne({ guildID: message.guild.id, komutAd: args[1] }, { $push: { roller: role.id } }).exec();
                }
            }
            if (["kisiler"].includes(args[2])) {
                ...
                if (ozelKomutDATA.kisiler.includes(kisi.id)) {
                    ...
                    await ozelKomut.updateOne({ guildID: message.guild.id, komutAd: args[1] }, { $pull: { kisiler: kisi.id } }).exec();
                } else {
                    message.reply("başarılı bir şekilde etiketlediğiniz kişiyi ekledim.");
                    await ozelKomut.updateOne({ guildID: message.guild.id, komutAd: args[1] }, { $push: { kisiler: kisi.id } }).exec();
                }
            }
            if (["sorumluluk"].includes(args[2])) {
                ...
                if (data.some(veri => veri.Ad.includes(args[3]))) {
                    ...
                    await ozelKomut.updateOne({ guildID: message.guild.id, komutAd: args[1] }, { $pull: { sorumluluk: args[3] } }).exec();
                } else {
                    message.reply("başarılı bir şekilde sorumluluk rolünü ekledim.");
                    await ozelKomut.updateOne({ guildID: message.guild.id, komutAd: args[1] }, { $push: { sorumluluk: args[3] } }).exec();
                }
            }
            if (["yetkilialım"].includes(args[2])) {
                if (ozelKomutDATA.YetkiliROL == true) {
                    await ozelKomut.updateOne({ guildID: message.guild.id, komutAd: args[1] }, { $set: { YetkiliROL: false, YetkiliData: [] } }).exec();
                    message.reply("Başarılı bir şekilde bu komutu global rol olarak tanımladınız.");
                } else {
                    await ozelKomut.updateOne({ guildID: message.guild.id, komutAd: args[1] }, { $set: { YetkiliROL: true } }).exec();
                    message.reply("Başarılı bir şekilde bu kodu yetkili alım kodu yaptınız.");
                }
            }
        }
        if (["sil"].includes(sec)) {
...
─────────────────────
**Kişiler:**   
${data2.kisiler.length > 0 ? data2.kisiler.map(x => `<@${x}>`).join(', ') : "Veri yoktur."}
─────────────────────
**Yetkili Rolleri:**   
${data2.roller.length > 0 ? data2.roller.map(x => `<@&${x}>`).join(', ') : "Veri yoktur."}
─────────────────────
**Sorumluluk Rolleri:**   
        ...
        if (!sec) {
            ...
            message.channel.send({
                embeds: [new EmbedBuilder()
                    ...
                    .setTimestamp()
                    .setFooter({ text: client.ayarlar.footer })
                    .setDescription(`Aşağıdaki özel komutları daha detaylı bir şekilde görmek için aşağıdaki komutu yazınız.\n\`.özelkomut bak <komutAdı>\`\n${göster}`)
                ]
            });
        }
    } else return;
