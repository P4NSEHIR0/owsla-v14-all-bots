...
module.exports.run = async (client, message, args, durum, kanal) => {
    ...
    if (await client.permAyar(message.author.id, message.guild.id, "jail") || durum || message.member.roles.cache.get("768977391963209729")) {
        ...
        await otokayit.deleteOne({ userID: target.id }).exec();
        target.setNickname(`${target.user.username.includes(tag) ? tag : tag2 ? tag2 : tag} İsim | Yaş`);
        target.roles.set(unRegisterRol).then(x => message.react(client.emojis.cache.find(x => x.name === "stfu_tik"))).catch(y => message.react(client.emojis.cache.find(x => x.name === "stfu_iptal")));
    } else {
        return;
    }
};
