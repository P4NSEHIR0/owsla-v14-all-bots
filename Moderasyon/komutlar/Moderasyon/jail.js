...
module.exports.run = async (client, message, args, durum, kanal) => {
    ...
    if (sec == "setup") {
        ...
        if (message.guild.members.cache.some(member => conf.sahip.some(sahip => member === sahip)) || message.member.permissions.has(8) || message.author.id === message.guild.ownerId) {
            await sunucuayar.findOne({ guildID: message.guild.id }, async (err, data) => {
                if (args[1] == "yetki") {
                    ...
                    data.JAILAuthorized = select;
                    data.save().then(y => message.react(client.emojis.cache.find(res => res.name === "stfu_tik")));
                }
                if (args[1] == "kanal") {
                    let select = message.mentions.channels.first();
                    if (!select) return message.react(client.emojis.cache.find(res => res.name === "stfu_iptal"));
                    data.JAILChannel = select.id;
                    data.save().then(y => message.react(client.emojis.cache.find(res => res.name === "stfu_tik")));
                }
                if (args[1] == "limit") {
                    let select = Number(args[2]);
                    if (!select) return message.react(client.emojis.cache.find(res => res.name === "stfu_iptal"));
                    data.JAILLimit = select;
                    data.save().then(y => message.react(client.emojis.cache.find(res => res.name === "stfu_tik")));
                }
            });
        } else {
            return message.reply("Bu komutu kullanabilmek için YÖNETİCİ - Sunucu Sahibi olmanız gerekiyor");
        }
    }
    ...
    if (await client.permAyar(message.author.id, message.guild.id, "jail") || durum) {
        if (jailSorumlusu.length >= 1 && client.channels.cache.get(jailLogKanal) && jailLimit >= 1) {
            ...
            if (target.roles.cache.get(jailROL)) return message.react(client.emojis.cache.find(res => res.name === "stfu_iptal"));
            ...
            return client.Embed(message.channel.id, "Lütfen jail komudunun kurulumunu tamamlayınız `" + conf.prefix[0] + "jail setup` yazarak kurunuz!");
        }
    } else {
        return;
    }
...
async function banSistemi(message, client, jailLogKanal, target, cezaID, jailROL, booster, reason) {
    let embedDescription = `${target} Üyesi Sunucudan **${reason}** sebebiyle ${message.author} Tarafından jail cezası yedi! **Ceza Numarası:** (\`#${cezaID + 1}\`)`;
