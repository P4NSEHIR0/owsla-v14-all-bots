...
const yetkiDATA = require("../../models/yetkili");
const hanedan = require("../../models/hanedanlik");
const Stat2 = require("../../models/stats");
const limit = new Map();
...
module.exports.run = async (client, message, args, durum, kanal) => {
    ...
    if (durum) {
        if (args[0] === "top") {
            let data = await yetkiDATA.find({ Durum: "stat" });
            let kayitcilar = {};
            data.forEach((value) => {
        ...
        if (args[0] === "bak") {
            ...
            let data = await yetkiDATA.find({ authorID: kisi.id, Durum: "stat" });
            let embed = new EmbedBuilder()
                .setColor("RANDOM")
                .setTimestamp()
                .setFooter({ text: client.ayarlar.footer })
                .setAuthor({ name: message.author.tag, iconURL: message.author.avatarURL({ dynamic: true }) })
                .setDescription(data.filter(veri => message.guild.members.cache.get(veri.userID) && message.guild.members.cache.get(veri.userID).user.username.includes(sdata.TAG))
                .map((veri, index) => `<@!${veri.userID}>: ${moment(veri.Tarih).locale("tr").format("LLL")}`).join("\n"));
        ...
        if (args[0] === "sorgula") {
            ...
            let data = await yetkiDATA.findOne({ userID: kisi.id, Durum: "stat" });
        ...
        if (Date.now() - target.user.createdAt <= 1000 * 60 * 60 * 24 * 7) return message.reply("Lütfen 7 günden kısa sürede açılan hesapları yetkili yapmayı deneyiniz.").then(x => setTimeout(() => x.delete(), 5000));
        ...
        await yetkiDATA.findOne({ userID: target.id, Durum: "stat" }, async (err, res) => {
            if (!res) return message.reply(`Bu komutu sadece 5 dakika içerisinde yetki verilen kullanıcılara kullanabilirsiniz.`);
            if (res.authorID != "x") return message.reply(`Yetki aldırmaya çalıştığın üye farklı bir yetkili tarafından zaten yetkiye alınmış!`);
        ...
        return message.reply("Bu Komutu Kullanabilmek için Yönetici ya da Kayıt Sorumlusu olman gerekiyor!");
    }
};
