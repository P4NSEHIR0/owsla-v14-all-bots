...
module.exports = async (oldUser, newUser) => {
  if (oldUser.username !== newUser.username) {
    ...
    let otoreg = await otokayit.findOne({ userID: user.id });
    ...
    if (newUser.username.includes(ayarlar.TAG) && !user.roles.cache.has(ayarlar.TEAM)) {
      if (user.manageable && ayarlar.TAG2) {
        await user.setNickname(user.displayName.replace(ayarlar.TAG2, ayarlar.TAG)).catch(() => {});
      }
      if (ayarlar.TEAM) await user.roles.add(ayarlar.TEAM).catch(() => {});
      if (ayarlar.TAGLOG && log) {
        log.send(`
...
Önce: ${oldUser.tag} | Sonra: ${newUser.tag}`).catch(() => {});
      ...
      if (user.manageable && ayarlar.TAG2) {
        await user.setNickname(user.displayName.replace(ayarlar.TAG, ayarlar.TAG2)).catch(() => {});
      }
      if (ayarlar.TEAM) {
        let ekipRol = guild.roles.cache.get(ayarlar.TEAM);
        if (client.ayarlar.taglıAlım === true && !user.roles.cache.has(ayarlar.BOOST)) {
          await tagSaldi(user.id);
          return await user.roles.set(ayarlar.UNREGISTER);
        }
        if (user.roles.cache.has("846064661446262795")) {
...
Bırakmadan önceki yetkileri:\n${user.roles.cache.filter(rol => ekipRol.position <= rol.position && !rol.managed).map(x => `<@&${x.id}>`).join(', ')}
          `);
        }
...
Önce: ${oldUser.tag} | Sonra: ${newUser.tag}`).catch(() => {});
      ...
      await user.roles.set(user.roles.cache.has(boostRol) ? [boostRol, yasaklıtagRol] : [yasaklıtagRol]).catch(() => {
        console.log("Yasaklı tag güncelleme kodunda rol verilirken bir hata meydana geldi");
      });
    } else if (!yasaklıtag.some(tag => newUser.username.includes(tag)) && user.roles.cache.has(yasaklıtagRol)) {
      if (otoreg) {
        if (user.manageable) await user.roles.set(user.roles.cache.has(boostRol) ? [...otoreg.roleID, boostRol] : otoreg.roleID);
...
async function tagSaldi(memberID) {
  await tagData.deleteMany({ userID: memberID });
  await yetkiDATA.deleteMany({ userID: memberID });
