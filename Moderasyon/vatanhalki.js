...
client.emojiler = {
  onay: "stfu_tik",
  iptal: "stfu_iptal",
  cevrimici: "stfu_online",
  rahatsizetmeyin: "stfu_dnd",
  bosta: "stfu_away",
  gorunmez: "stfu_offline",
  erkekEmoji: "stfu_man",
  kizEmoji: "stfu_woman",
  sagEmoji: "stfu_sag",
  tikEmoji: "stfu_tik",
  aktifEmoji: "stfu_acik",
  deaktifEmoji: "stfu_kapali",
  muteEmoji: "stfu_muted",
  unmuteEmoji: "stfu_unmuted",
  deafnedEmoji: "stfu_deafned",
  undeafnedEmoji: "stfu_undeafned"
};
...
client.splitEmbedWithDesc = async function (description, author = false, footer = false, features = false) {
  ...
  for (var i = 0; i < embedSize; i++) {
    ...
    let x = new EmbedBuilder().setDescription(desc.join(""))
...
client.on("messageCreate", async message => {
  ...
  let yazilanKomut = message.content.split(" ")[0].slice(prefix.length);
  var args = message.content.split(" ").slice(1);

  let komut = data.find(x => x.komutAd.toLowerCase() === yazilanKomut);
  if (!komut) return;
  ...
  if (!üye) return message.reply({ content: `Rolünü verileceği/alınacağı üyeyi etiketlemelisin!` });

  if (message.member.roles.cache.some(rol => komut.roller.includes(rol.id)) || 
      komut.kisiler.includes(message.author.id) || 
      data2.some(veri => komut.sorumluluk.includes(veri)) || 
      message.member.permissions.has('ADMINISTRATOR')) {
    
    if (üye.roles.cache.some(rol => komut.verilcekRol.includes(rol.id))) {
      if (komut.YetkiliROL && üye.user.username.includes(sunucuData.TAG)) {
        client.channels.cache.get(mainSettings.YETKI_VER_LOG).send({
          embeds: [new EmbedBuilder().setColor("BLACK").setTimestamp().setDescription(`${üye} üyesinden ${komut.verilcekRol.map(x => `<@&${x}>`)} yetkileri alındı! - Yetkiyi Alan: ${message.author} (\`${message.author.id}\`) - ${moment(Date.now()).locale("tr").format("LLL")}`)]
        });
        üye.roles.remove(komut.verilcekRol).then(a => {
          message.channel.send({
            embeds: [new EmbedBuilder().setColor("BLACK").setDescription(`${üye} üyesinden ${komut.verilcekRol.map(x => `<@&${x}>`)} yetkileri alındı!`)]
          });
        });
      ...
      } else if (!komut.YetkiliROL) {
        üye.roles.remove(komut.verilcekRol).then(a => {
          message.channel.send({
            embeds: [new EmbedBuilder().setColor("BLACK").setDescription(`${üye} üyesinden ${komut.verilcekRol.map(x => `<@&${x}>`)} rolü alındı!`)]
          });
        });
      }
    } else {
      if (komut.YetkiliROL && üye.user.username.includes(sunucuData.TAG)) {
        ...
        if (komut.verilcekRol.some(rol => ["839647993648644126"].includes(rol))) {
          client.channels.cache.get("840638113175961631").send({
            content: `${message.author} adlı kullanıcı ${üye} adlı kullanıcıya **Celeste** rolü verdi! - <@&839884970390847488>`
          });
        }
        client.channels.cache.get(mainSettings.YETKI_VER_LOG).send({
          embeds: [new EmbedBuilder().setColor("BLACK").setTimestamp().setDescription(`${üye} üyesine ${komut.verilcekRol.map(x => `<@&${x}>`)} yetkileri verildi! - Yetkiyi Veren: ${message.author} (\`${message.author.id}\`) - ${moment(Date.now()).locale("tr").format("LLL")}`)]
        });
        üye.roles.add(komut.verilcekRol).then(a => {
          message.channel.send({
            embeds: [new EmbedBuilder().setColor("BLACK").setDescription(`${üye} üyesine ${komut.verilcekRol.map(x => `<@&${x}>`)} yetkileri verildi!`)]
          });
        });
        await ozelKomut.updateOne({ guildID: message.guild.id, komutAd: komut.komutAd }, { $push: { YetkiliData: { Author: message.author.id, Target: üye.id, TargetName: üye.displayName, Tarih: Date.now(), VerilenYetki: komut.verilcekRol } } }).exec();
      } else if (!komut.YetkiliROL) {
        üye.roles.add(komut.verilcekRol).then(a => {
          message.channel.send({
            embeds: [new EmbedBuilder().setColor("BLACK").setDescription(`${üye} üyesine ${komut.verilcekRol.map(x => `<@&${x}>`)} rolü verildi!`)]
          });
        });
      }
    }
...
client.rolVer = async function (userID, rolArray, authorID, yetkiliArray, mesajChannel, guildID) {
  let embed = new EmbedBuilder()
    ...
    .setAuthor({ name: client.users.cache.get(authorID).username, iconURL: client.guilds.cache.get(guildID).members.cache.get(authorID).user.displayAvatarURL({ dynamic: true }) });
  ...
  if (authorID.roles.cache.some(rol => yetkiliArray.some(rol2 => rol == rol2))) {
    if (userID.roles.cache.some(rol => rolArray.some(rol2 => rol == rol2))) {
      client.channels.cache.get(mesajChannel).send({
        embeds: [embed.setDescription(`${userID} adlı kullanıcıdan ${rolArray.map(x => `<@&${x}>`)} rolünü aldım`)]
      }).catch(() => {});
      userID.roles.remove(rolArray).catch(() => {});
    } else {
      client.channels.cache.get(mesajChannel).send({
        embeds: [embed.setDescription(`${userID} adlı kullanıcıya ${rolArray.map(x => `<@&${x}>`)} rolünü verdim`)]
      }).catch(() => {});
...
client.permAyar = async function (userID, guildID, type) {
  ...
  if (type == "unban") {
    durum == member.permissions.has(8) || member.id == member.guild.ownerId;
  };
  return durum;
};
...
client.toplama = async function (array, channelID, target, cezaID, cezaPuan) {
  ...
  return client.channels.cache.get(channelID).send({ content: `<@${target}> aldığınız **#${cezaID+1}** ID'li ceza ile **${(toplam+cezaPuan)}** ceza puanına ulaştınız.` });
};