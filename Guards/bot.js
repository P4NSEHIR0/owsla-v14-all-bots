...
RoleGuard.on("ready", async () => {
    ...
    let kanal = RoleGuard.channels.cache.filter(x => x.type === "GUILD_VOICE" && x.id === Config.BotSesKanal);
    setInterval(() => {
...
let veri = await SafeMember.findOne({ guildID: message.guild.id }) || {
    "Full": [],
    "RoleAndChannel": [],
    "Role": [],
    "Channel": [],
...
`)
                ...
                if (!dats && dats.type !== "voice") return message.reply({ content: "Lütfen bir Kişi & Rol & Ses Kanal ID'si giriniz veya Kişi & Rol & Ses Kanal Etiketleyiniz!" });
...
`)
                ...
                if (veri.Bot.includes(dats.id)) {
                    ...
                    message.channel.send({ content: `Başarılı bir şekilde ${dats} verisini güvenli listeye ekledin! (**Bot**)` });
                    await SafeMember.updateOne({
                        guildID: message.guild.id
                    }, {
                        $push: {
...
${veri.ChatG.length > 0 ? veri.ChatG.map(x => `${message.guild.members.cache.get(x) ? message.guild.members.cache.get(x) : message.guild.roles.cache.get(x) ? message.guild.roles.cache.get(x) : message.guild.channels.cache.get(x)}`).join("\n") : "Herhangi bir üye & rol güvenliye eklenmedi!"}`)
        ...
        return message.channel.send({ embeds: [embed] })
            ...
            await SafeMember.updateOne({guildID: message.guild.id}, {$pull: {SafeRole: rol.id}}, {upsert: true});
              message.channel.send({ content: embed.setDescription(`${rol}, ${message.author} tarafından güvenli rol listesinden kaldırıldı!`) });
            } else {
                await SafeMember.updateOne({guildID: message.guild.id}, {$push: {SafeRole: rol.id}}, {upsert: true});
              message.channel.send({ content: embed.setDescription(`${rol}, ${message.author} tarafından güvenli rol listesinden eklendi!`) });
            };
          };
    }
    ...
    if (checkPermission(RoleGuard, message.author.id, "full") && command === "!yt") {
        ...
        if (sec === "aç") {
            if (veri.Permissions && veri.Permissions.length > 0) {
                for (let i = 0; i < veri.Permissions.length; i++) {
                    const elm = veri.Permissions[i];
                    let role = message.guild.roles.cache.get(elm.id);
                    if (role) {
                        role.setPermissions(elm.permission).catch(console.error);
                    }
                }
                message.channel.send({ content: `Başarılı bir şekilde yetkileri açtınız!` });
            } else {
                return message.reply({ content: "Lütfen mevcut izinleri kaydediniz!" });
            }
        }
        ...
        if (sec === "kapat") {
            if (veri.Permissions && veri.Permissions.length > 0) {
                for (let i = 0; i < veri.Permissions.length; i++) {
                    const elm = veri.Permissions[i];
                    let everyoneRole = message.guild.roles.cache.find(x => x.name === "@everyone");
                    let permission = everyoneRole.permissions.has(8) ? everyoneRole.permissions.bitfield : 0;
                    let role = message.guild.roles.cache.get(elm.id);
                    if (role) {
                        role.setPermissions(permission).catch(console.error);
                    }
                }
                message.channel.send({ content: `Başarılı bir şekilde yetkileri kapattınız!` });
            } else {
                return message.reply({ content: "Lütfen mevcut izinleri kaydediniz!" });
            }
        }
        ...
        if (sec === "yükle" && Config.BotOwner.includes(message.author.id)) {
            message.guild.roles.cache.filter(role => izinler.some(perm => role.permissions.has(perm)) && !role.managed).forEach(role => {
                data.push({
                    id: role.id,
            ...
            const embed = new EmbedBuilder()
                .setColor(`#${randomInt(0xFFFFFF).toString(16).padStart(6, '0')}`) // Rastgele renk
    ...
    if (Config.BotOwner.includes(message.author.id) && command === "!silinenroller") {
        try {
            ...
            let formattedData = veri.reverse().map(dat => `Eski Rol: ${dat.oldRoleID} -> Yeni Rol: ${dat.newRoleID} => ${moment(dat.Date).format("LLL")}`).join("\n");
            message.channel.send({ content: formattedData, code: "md", split: true });
        } catch (err) {
            console.error(err);
            message.channel.send("Verileri alırken bir hata oluştu.");
        }
    }
...
ChannelGuard.on("ready", () => {
    ...
    let kanal = ChannelGuard.channels.cache.filter(x => x.type === "GUILD_VOICE" && x.id === Config.BotSesKanal);
    setInterval(() => {
      const oynuyor = Config.BotDurum;
      const index = Math.floor(Math.random() * (oynuyor.length));
...
OtherGuard.on("ready", () => {
    ...
    let kanal = OtherGuard.channels.cache.filter(x => x.type === "GUILD_VOICE" && x.id === Config.BotSesKanal);
    setInterval(() => {
      const oynuyor = Config.BotDurum;
      const index = Math.floor(Math.random() * (oynuyor.length));
...
OtherGuard.on("messageCreate", async (message) => {
    ...
    if (await checkPermission(OtherGuard, message.author.id, "chatguard") || message.author.id === message.guild.ownerId) return;
    ...
    if (ms.length > 5 && !["game","owo","fun","gif","t-sustum"].some(ver => message.channel.name.includes(ver))) {
        ...
        message.reply({ content: "Spam yaptığın için sunucumuzda 3 dakika boyunca susturuldun!" });
        setTimeout(() => {
    ...
    if (kufurler.some(word => args.some(c => word.toLowerCase() == c.toLowerCase())) && !["t-sustum"].some(ver => message.channel.name.includes(ver))) {
		...
		message.delete().then(msg => {
          message.reply({ content: " etme eyleme kardeşim günah ya." }).then(m => m.delete({
            timeout: 2000
          })).catch(err => {});
        });
...
OtherGuard.on("messageUpdate", async (oldMessage, newMessage) => {
    ...
    if (await checkPermission(OtherGuard, oldMessage.author.id, "chatguard") || oldMessage.author.id === oldMessage.guild.ownerId) return;
    const kelime = ["discord.gg", "discord.me", "discordapp.com", "discord.io", "discord.tk", ".gg/", ".gg"];
    ...
    if (kufurler.some(word => args.some(c => word.toLowerCase() == c.toLowerCase())) && !["sus"].some(ver => newMessage.channel.name.includes(ver))) {
      newMessage.delete().then(message => {
        newMessage.reply({ content: " etme eyleme kardeşim günah ya." }).then(m => m.delete({
          timeout: 2000
        })).catch(err => {});
      });
    }
...
async function checkPermission(bots, id, type) {
    ...
    if (!res) {
        ...
        if (type == "full") {
            ...
            if (res.Channel.some(uye => uye == member?member.id:false  || member ? member.roles.cache.has(uye) : false || member ? member.me.voice ? member.me.voice.channel.id == uye : false : false)) return true;
