...
client.on("ready", async () => {
    ...
    setInterval(async () => {
        ...
        let embed = new EmbedBuilder()
            ...
            client.channels.cache.get(mainSettings.GörevSystem.KanalID).messages.fetch(mainSettings.GörevSystem.MesajID).then(m => m.edit({ embeds: [embed] }));
    }, 1000 * 60);
            ...
            if (!res) {
                ...
                if (Date.now() - res.Date >= 1000 * 60 * 60 * 24 * 1) {
                    ...
                    setTimeout(async () => {
                        ...
                        let VoiceChannel = client.guilds.cache.get(mainSettings.sunucuId).channels.cache.filter(chan => chan.type == "GUILD_VOICE" && kategoriler.includes(chan.parentID) && !yasaklıkanal.includes(chan.id)).map(channel => channel.id)
                        let MessageChannel = client.guilds.cache.get(mainSettings.sunucuId).channels.cache.filter(chan => chan.type == "GUILD_TEXT" && messageKategori.includes(chan.id)).map(channel => channel.id)
                        client.channels.cache.get(dailyData.DailyMission.logChannel).send({ content: `\`\`\`${client.guilds.cache.get(mainSettings.sunucuId).name} ${moment(Date.now()).locale("tr").format("LLL")} tarihinde dağıtılan günlük görevler;\`\`\`` });
                        veri.forEach((user, index) => {
                            setTimeout(async () => {
                                if (index >= veri.length) return client.channels.cache.get(`${message.channel.id}`).send({ content: `Başarılı bir şekilde tüm görevler dağıtıldı.` });
                                ...
                                if (user.gorev == "ses") {
                                    ...
                                    client.channels.cache.get(dailyData.DailyMission.logChannel).send({ content: `<@${user.user}> Bugün <#${VoiceRandom}> kanalında \`${miktarlar}\` dakika ses aktifliği yapman gerekiyor.` });
                                    randMiss.updateOne({
                                        userID: user.user
                                    }, {
                                        $set: {
                                ...
                                if (user.gorev == "mesaj") {
                                    ...
                                    client.channels.cache.get(dailyData.DailyMission.logChannel).send({ content: `<@${user.user}> Bugün <#${MessageRandom}> kanalında \`${miktarlar}\` adet mesaj yazman gerekiyor.` });
                                    randMiss.updateOne({
                                        userID: user.user
                                    }, {
                                        $set: {
                                ...
                                if (user.gorev == "taglı") {
                                    client.channels.cache.get(dailyData.DailyMission.logChannel).send({ content: `<@${user.user}> Bugün \`${miktarlar}\` adet taglı üye çekmen gerekiyor.` });
                                    randMiss.updateOne({
                                        userID: user.user
                                    }, {
                                        $set: {
                                ...
                                if (user.gorev == "teyit") {
                                    client.channels.cache.get(dailyData.DailyMission.logChannel).send({ content: `<@${user.user}> Bugün \`${miktarlar}\` adet teyit yapman gerekiyor.` });
                                    randMiss.updateOne({
                                        userID: user.user
                                    }, {
                                        $set: {
                                ...
                                if (user.gorev == "davet") {
                                    client.channels.cache.get(dailyData.DailyMission.logChannel).send({ content: `<@${user.user}> Bugün \`${miktarlar}\` adet davet yapman gerekiyor.` });
                                    randMiss.updateOne({
                                        userID: user.user
                                    }, {
                                        $set: {
...
message.channel.send({ content: `${message.author} ${rand}` })
}
});
...
client.on("ready", async () => {
    try {
        ...
        let kanal = client.channels.cache.filter(x => x.type === "GUILD_VOICE" && x.id === mainSettings.botSesID);
        setInterval(() => {
        ...
        vmuted.forEach(async memberdata => {
            ...
            if (!sunucu.members.cache.has(memberdata.userID)) {
                ...
                if (!member) return;
                    ...
                    await member.me.voice.setMute(false).catch(() => {});
                    vmuteInterval.deleteOne({
                        userID: memberdata.userID
                    }).exec()
            }
