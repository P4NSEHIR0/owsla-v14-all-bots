...
module.exports.run = async (client, message, args, durum, kanal) => {
    ...
    if (gunluk !== null && timeout - (Date.now() - gunluk) > 0) {
        ...
        message.channel.send({ content: `:stopwatch: **|** Hata! **${message.author.username}** Bu komutu ${time.seconds} saniye sonra kullanabilirsin.` })
	} else {
...
const symbols = [
    ...
    if (!sec) return message.channel.send({ content: `:no_entry: | **${message.author.username},** Lütfen bir bahis değeri giriniz!!` })
	...
	if (sec == "all" && data.para > 0) {
		...
		embed.addFields(points ? 'KAZANDIN!' : 'KAYBETTİN!',  points ? `Tebrikler **${parseInt(payout)}** miktar para kazandın!` : '**TÜH!** Kaybettin bi dahaki sefer kazanman dileğiyle (:')
    ...
    if (data.para > Number(args[0]) && Number(args[0]) <= 50000 && Number(args[0]) > 0) {
        ...
        message.channel.send({ content: `:no_entry: | **${message.author.username}**, Yeterli miktar da paran yoktur! (Max: 50.000 Tutarında Oynayabilirsin)` })
    }
