...
exports.run = async function (client, message, args, durum, kanal) {
    ...
    if (!message.member.voice.channel) return message.reply({ content: `Hata: Bu komutu kullanabilmek için bir ses kanalında olmanız gerekiyor` })
...
Bu oyun toplamda **${oyun[0].Gun} günde** bitti! ${oyun[0].Gun == 4 ? "(`Oyun gidişatı orta seviye`)" : oyun[0].Gun >= 6 ? "(`Bu oyun bitmek bilmedi güzeldi!`)" : "(\`Oyun hızlı bitti :(\`)" }`)
        ...
        if (!data.map(x => x.Channel).some(kanal => kanal === message.member.voice.channel.id)) return message.channel.send({ content: new EmbedBuilder().setColor("RED").setDescription(`Oyunu başlatmak için bir lobi oluşturmalısınız! (\`${prefix}vkstart\`)`) })
