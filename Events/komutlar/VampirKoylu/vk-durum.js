...
exports.run = async function (client, message, args, durum, kanal) {
    ...
    if (!message.member.roles.cache.has(vkyonetici) && !message.member.permissions.has(PermissionsBitField.Flags.Administrator)) {
        return message.reply({ content: "Yönetici ya da VK Sorumlusu olman gerekiyor!" });
    }
    if (!message.member.voice.channel) {
        return message.reply({ content: `Hata: Bu komutu kullanabilmek için bir ses kanalında olmanız gerekiyor` });
    }
...
`);
        ...
        if (!data.map(x => x.Channel).some(kanal => kanal === message.member.voice.channel.id)) {
            return message.channel.send({
                embeds: [new EmbedBuilder().setColor("Red").setDescription(`Oyunu başlatmak için bir lobi oluşturmalısınız! (\`${prefix}vkstart\`)`)]
            });
        }
